"""
File describes Warping Neural ODE
"""
import time
import numpy as np
...
class STODEGradNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        # To control number of plotted, and avoid plotting reverse
        self.ode_step_size = args.ode_step_size
        self.plot_reverse = False  #if true we also plot reverse ODE
    ...
    def forward(self, t, y):
        ...
        if self.plot_all:
            plot_inds = range(self.warpimg.shape[0])
        else:
            plot_inds = [0]
        self.nfe += 1

        if self.plot_seq and not y.requires_grad and \
           not self.plots_path is None and\
           (self.plot_reverse or\
            (not self.plot_reverse and self.nfe <= 1/self.ode_step_size)):
            for plot_ind in plot_inds:
                plots_path = "%s/vf_seq/%s_%s/" % (self.plots_path,
                                                   self.model_name, plot_ind)
...
class STODE(nn.Module):
    def __init__(self, args, device):
        ...
        self.model_name = ""
        self.plots_path = args.plots_path
        self.plot_last_vf = args.plot_last_vf
        self.plot_all = args.plot_all
    ...
    def forward_warpode(self,
        ...
        if not backward and normalize and not flow.requires_grad and \
           self.plot_last_vf:
            if self.plot_all:
                plot_inds = range(y_hat.shape[0])
            else:
                plot_inds = [0]

            for plot_ind in plot_inds:
                plots_path = "%s/vf/%s_%s/" % (self.plots_path,
                                               self.model_name, plot_ind)
                os.makedirs(plots_path, exist_ok=True)
                ...
                flow_reverse = self.de_solver(flow0,
                                              torch.tensor([1, 0]).to(x))
                ...
                # Plot final flow
                flow_ = flow[
                    plot_ind, :, ::step, ::step].detach().cpu().numpy()
                ...
                y_ = y_hat_grid[plot_ind, 0].cpu().data.numpy()
                x_ = y_hat_grid[plot_ind, 1].cpu().data.numpy()
                ...
                plt.imshow(
                    np.minimum(
                        0 * y_hat[plot_ind, 0].detach().cpu().numpy() +
                        1 * x[plot_ind, 0].cpu().numpy(),
                        1,
                    ))
                plt.quiver(x_,
                           y_,
                ...
                flow_ = flow_reverse[
                    plot_ind, :, ::step, ::step].detach().cpu().numpy()
                y_ = reverse_grid[plot_ind, 0].cpu().data.numpy()
                x_ = reverse_grid[plot_ind, 1].cpu().data.numpy()
                u = -flow_[1]
                v = -flow_[0]
                plt.quiver(x_,
                           y_,
                ...
                if y is not None:
                    plt.imshow(y[plot_ind, 0].cpu().numpy() -
                               y_hat[plot_ind, 0].detach().cpu().numpy(),
                               interpolation='bilinear',
                               cmap='jet')
                ...
                # plt.imshow(y_hat[plot_ind, 0].detach().cpu().numpy(),
                #            interpolation='bilinear',
                #            cmap='jet')
                plt.imshow((y_hat[plot_ind].cpu().numpy().transpose(
                    (1, 2, 0)) * 255).astype('uint8'),
                           interpolation='bilinear',
                           cmap='jet')
                plt.clim(0, 1)
                ...
                plt.imshow(x[plot_ind, 0].cpu().numpy() -
                           y_hat[plot_ind, 0].detach().cpu().numpy(),
                           interpolation='bilinear',
                           cmap='jet')
                ...
                plt.imshow(x_reverse[plot_ind, 0].cpu().numpy(),
                           interpolation='bilinear',
                           cmap='jet')
                plt.clim(0, 1)
                plt.colorbar()
                ...
                plt.imshow(x[plot_ind, 0].cpu().numpy() -
                           x_reverse[plot_ind, 0].cpu().numpy(),
                           interpolation='bilinear',
                           cmap='jet')
                plt.clim(0, 1)
                plt.colorbar()
